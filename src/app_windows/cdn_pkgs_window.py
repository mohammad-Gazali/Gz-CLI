...
cdn_choices_menu = [(pkg.get_full_name().capitalize(), pkg) for pkg in PACKAGES_LIST]
...
def print_menu(stdscr, selected_row_idx, user_choices):
    ...
    for idx, item in enumerate(cdn_choices_menu):
        x = 2
        y = idx + 2
...
def cdn_packages_main(stdscr):
    ...
    while True:
        ...
        if key == curses.KEY_UP and current_row == 0:
            current_row = len(cdn_choices_menu) - 1

        elif key == curses.KEY_DOWN and current_row == len(cdn_choices_menu) - 1:
            current_row = 0
